...
9. Render only through the exact commands below. Do not call `--help` between stages when these commands succeed. The fixed Plain Deep template includes the mountain-route cover, AiDesignLab.io logo/credit, stronger Traditional Chinese typography, card contents, eight-domain dashboard, vertical timeline, pale-blue bands, headers, footers and dynamic body pagination. Never approximate it with `render_apple_pdf.py`. Run privacy scanning only on `DELIVERY_DIR`; never scan `private_cases/CASE`, because calculation artifacts intentionally contain birth data.
