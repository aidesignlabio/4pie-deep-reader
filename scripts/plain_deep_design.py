...
def header(c,page,kicker,title,sub='',footer='4PIE 深讀報告'):
    ...
    c.setStrokeColor(LINE); c.line(18*MM,14*MM,W-18*MM,14*MM); para(c,footer,18*MM,7*MM,60*MM,5*MM,'small'); para(c,f'{page:02d}',W-27*MM,7*MM,9*MM,5*MM,'small')

def end(c): c.showPage()

def cover(c,title,subject,thesis,generated,language='zh-TW'):
    ...
    systems='Western Astrology · Zi Wei Dou Shu · Vedic Astrology · Bazi' if language=='en' else '西洋占星 · 紫微斗數 · 吠陀占星 · 八字'
    date_label='Generated' if language=='en' else '生成日期'
    c.setFont('TCB',10); c.setFillColor(BLUE); c.drawString(18*MM,H-80*MM,'PERSONAL STRATEGY REPORT'); para(c,thesis,18*MM,H-111*MM,125*MM,20*MM,'h3'); para(c,f'{subject}\n{systems}\n{date_label} {generated}',18*MM,31*MM,155*MM,25*MM,'small'); para(c,'AiDesignLab.io',W-66*MM,25*MM,48*MM,7*MM,'small'); end(c)
