...
def main():
    ...
    # 6. 最小计算测试
    if all_ok:
        print(f"\n── 合成個案實算測試 (2000-01-01 UTC) ──")
        test_code = f"""
import sys
sys.path.insert(0, r"{SCRIPT_DIR}")
from engine import calculate_full_chart
chart = calculate_full_chart(2000, 1, 1, 12, 0, 0.0, 0.0, "UTC")
assert chart.get('lagna', {{}}).get('sign')
assert len(chart.get('planets', {{}})) >= 7
assert chart.get('dashas')
assert chart.get('sav')
print(f"Lagna={{chart['lagna']['sign']}} Planets={{len(chart['planets'])}} Dashas={{len(chart['dashas'])}}")
print("PASS")
"""
