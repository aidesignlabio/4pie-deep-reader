...
REQUIRED_PACKAGES = [
    ...
    ("PyYAML", ">=6.0", []),                          # 11. Contracts and schemas
    ("reportlab", ">=4.2", []),                       # 12. PDF renderer
...
def validate(venv_dir):
    ...
    checks = [
        ...
        ("reportlab", "import reportlab; print('OK')"),
        ("pypdf", "import pypdf; print('OK')"),
        ("Pillow", "from PIL import Image; print('OK')"),
        ("PyMuPDF", "import pymupdf; print('OK')"),
    ]
    ...
    # 2. Public API smoke test. Do not pin a historical celebrity or one
    # PyJHora-internal SAV total: those are brittle across upstream releases.
    scripts_dir = os.path.dirname(os.path.abspath(__file__))
    test_code = f"""
import sys, io
...
r = calculate_full_chart(2000, 1, 1, 12, 0, 0.0, 0.0, 'UTC')
assert r.get('lagna', {{}}).get('sign'), 'missing lagna'
assert len(r.get('planets', {{}})) >= 7, 'missing planets'
assert r.get('dashas'), 'missing dashas'
assert r.get('sav'), 'missing SAV payload'
print('VEDIC_SMOKE_OK')
"""
    result = subprocess.run([py, "-c", test_code], capture_output=True, text=True, timeout=60)
    if result.returncode == 0 and "VEDIC_SMOKE_OK" in result.stdout:
        log("吠陀公開 API 實算校驗通過", "OK")
    else:
        log(f"吠陀實算校驗失敗 (got: {result.stdout.strip()}, err: {result.stderr[-500:]})", "ERR")
        return False
